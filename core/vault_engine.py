...
class VaultEngine:
    ...
    def _inject_canaries(self, data: bytes, session_key: bytes) -> tuple[bytes, str]:
        ...
        canary_info = json.dumps(
            {"positions": positions, "values": canary_values},
            sort_keys=True
        )
    ...
    def _remove_canaries(self, data: bytes, session_key: bytes) -> bytes:
        ...
        n_canaries = 4
        positions  = [
            int.from_bytes(pos_seed[i*4:(i+1)*4], 'big') % max(len(data) - n_canaries + 1, 1)
            for i in range(n_canaries)
        ]
        data_list = list(data)
    ...
    def encrypt(self, plaintext: bytes, token: SessionToken) -> VaultBlob:
        ...
        print(f"[VAULT-X] Encrypted  ·  blob_id={blob.blob_id[:12]}...  "
    ...
    def decrypt(self, blob: VaultBlob, token: SessionToken) -> bytes:
        ...
        try:
            ...
            print(f"[VAULT-X] Decrypted  ·  blob_id={blob.blob_id[:12]}...  "
