...
class AuthGateway:
    ...
    def _record_failure(self, device_hash: str):
        ...
        if fail_count >= self.MAX_FAILURES:
            ...
            unlock_time = datetime.fromtimestamp(lockout_until).strftime("%Y-%m-%d %H:%M:%S")
            ...
            self._audit(
                "LOCKOUT_TRIGGERED",
                {"device": device_hash[:16], "fail_count": fail_count, "until": unlock_time},
            )
    ...
    def login(self) -> Optional[SessionToken]:
        ...
        if all_pass:
            ...
            self._audit(
                "LOGIN_SUCCESS",
                {"device": device_hash[:16], "session": token.session_id[:16], "layers": results},
            )

            print(f"\n  Session active for {self.vault.get_active_session_count()} session(s)")
            print(
                f"  Token expires: {datetime.fromtimestamp(token.expires_at).strftime('%H:%M:%S')}"
            )
            print("═" * 60 + "\n")
            return token
