...
def styled_btn(parent, text, cmd, color=ACCENT, width=18, small=False):
    ...
    b = tk.Button(
        ...
        bg=color, fg=BG_DARK if color in (ACCENT, ACCENT2, WARN) else TEXT_PRI,
...
def lbl(parent, text, fg=TEXT_PRI, size=11, bold=False, bg=None):
    return tk.Label(parent, text=text, bg=bg or BG_PANEL,
                    fg=fg, font=("Segoe UI", size, "bold" if bold else "normal"))

def entry(parent, show=None, width=30):
    e = tk.Entry(parent, bg=BG_INPUT, fg=TEXT_PRI, insertbackground=TEXT_PRI,
                 relief="flat", bd=0, font=FONT_BODY, show=show or "",
...
class LoginScreen(tk.Frame):
    ...
    def _build(self):
        ...
        # Logo
        tk.Label(outer, text="VAULT-X", bg=BG_DARK, fg=ACCENT,
                 font=("Segoe UI", 36, "bold")).pack(pady=(0,4))
        tk.Label(outer, text="Personal Data Vault  ·  5-Layer Authentication",
                 bg=BG_DARK, fg=TEXT_SEC, font=FONT_BODY).pack(pady=(0,24))

        # Card
        c = tk.Frame(outer, bg=BG_PANEL, bd=0,
                     highlightthickness=1, highlightbackground=BORDER)
        c.pack(ipadx=32, ipady=24, fill="both")

        # TOTP
        tk.Label(c, text="Layer A — TOTP Code", bg=BG_PANEL,
                 fg=TEXT_SEC, font=("Segoe UI",9,"bold")).pack(anchor="w", padx=4, pady=(16,2))
        ...
        # Passphrase
        tk.Label(c, text="Layer B — Passphrase", bg=BG_PANEL,
                 fg=TEXT_SEC, font=("Segoe UI",9,"bold")).pack(anchor="w", padx=4, pady=(0,2))
        ...
        for lid, lname, _ in self.LAYERS:
            ...
            tk.Label(row, text=f"[{lid}] {lname}", bg=BG_PANEL,
                     fg=TEXT_DIM, font=FONT_SMALL, width=20, anchor="w").pack(side="left")
            lbl_status = tk.Label(row, text="●  Waiting", bg=BG_PANEL,
                                  fg=TEXT_DIM, font=FONT_SMALL)
            lbl_status.pack(side="left", padx=8)
            self._layer_labels[lid] = lbl_status

        # Progress
        self._status_lbl = tk.Label(c, text="", bg=BG_PANEL,
        ...
        self._login_btn = styled_btn(btn_row, "🔓  LOGIN", self._start_login,
                                     color=ACCENT, width=20)
        self._login_btn.pack(side="left", padx=(0,8))
        styled_btn(btn_row, "⚙  Setup (First time)", self._on_setup,
                   color=BG_INPUT, width=20, small=True).pack(side="left")

        # Error label
        self._err_lbl = tk.Label(outer, text="", bg=BG_DARK,
    ...
    def _set_layer(self, lid, state):
        ...
        icons  = {"waiting": "●  Waiting", "running": "◌  Checking...",
                  "pass": "✓  PASS", "fail": "✗  FAIL"}
    ...
    def _start_login(self):
        ...
        self._auth_thread = threading.Thread(
            target=self._run_auth, args=(totp, pwd), daemon=True
        )
    ...
    def _run_auth(self, totp: str, pwd: str):
        try:
            ...
            if not gw.config.get("setup_complete"):
                self.after(0, lambda: self._err_lbl.config(
            ...
            def upd(lid, state, prog):
                ...
                self.after(0, lambda p=prog: self._status_lbl.config(
                    text=f"Verifying layer {lid}..."))
            ...
            # Layer C
            upd("C", "running", 50)
            b_pass, b_score = gw._behavioral.verify(timings=None if gw._behavioral._calibrated else [])
            results["C"] = b_pass
...
class DashboardPanel(tk.Frame):
    ...
    def _build(self):
        tk.Label(self, text="Dashboard", bg=BG_PANEL,
                 fg=TEXT_PRI, font=FONT_TITLE).pack(anchor="w", padx=20, pady=(20,4))
        tk.Label(self, text="Session overview and vault health",
                 bg=BG_PANEL, fg=TEXT_SEC, font=FONT_BODY).pack(anchor="w", padx=20, pady=(0,16))
        ...
        for key, title, val, color in stats:
            f = tk.Frame(row, bg=BG_CARD, bd=0,
                         highlightthickness=1, highlightbackground=BORDER)
            f.pack(side="left", expand=True, fill="both", padx=(0,12))
            tk.Label(f, text=title, bg=BG_CARD,
                     fg=TEXT_SEC, font=("Segoe UI",9,"bold")).pack(anchor="w", padx=14, pady=(12,2))
        ...
        self._sess_text = tk.Text(c, bg=BG_CARD, fg=TEXT_PRI,
                                  font=FONT_MONO_S, height=6, relief="flat",
        ...
        styled_btn(btn_row, "🔒  Encrypt File",
                   lambda: self._app.show_panel("vault"), ACCENT, 18).pack(side="left", padx=(0,8))
        styled_btn(btn_row, "🔓  Decrypt File",
                   lambda: self._app.show_panel("vault"), ACCENT2, 18).pack(side="left", padx=(0,8))
        styled_btn(btn_row, "📋  View Audit Log",
                   lambda: self._app.show_panel("audit"), BG_INPUT, 18).pack(side="left", padx=(0,8))
        styled_btn(btn_row, "🚨  Threat Monitor",
                   lambda: self._app.show_panel("monitor"), DANGER, 18).pack(side="left")
    ...
    def _refresh(self):
        ...
        if token:
            ...
            self._stat_frames["session_time"].config(
...
class FileVaultPanel(tk.Frame):
    ...
    def _build(self):
        tk.Label(self, text="File Vault", bg=BG_PANEL,
                 fg=TEXT_PRI, font=FONT_TITLE).pack(anchor="w", padx=20, pady=(20,4))
        tk.Label(self, text="Encrypt and decrypt your files",
                 bg=BG_PANEL, fg=TEXT_SEC, font=FONT_BODY).pack(anchor="w", padx=20, pady=(0,16))
        ...
        styled_btn(ef, "Browse", self._browse_encrypt,
                   BG_INPUT, 10, small=True).pack(side="left", padx=(8,0))

        btn_row = tk.Frame(ec, bg=BG_CARD)
        btn_row.pack(fill="x", padx=12, pady=(0,12))
        styled_btn(btn_row, "🔒  Encrypt File", self._encrypt, ACCENT, 20).pack(side="left")
        self._enc_status = tk.Label(btn_row, text="", bg=BG_CARD,
                                    fg=ACCENT2, font=FONT_SMALL)
        ...
        styled_btn(df, "Browse", self._browse_decrypt,
                   BG_INPUT, 10, small=True).pack(side="left", padx=(8,0))
        ...
        self._dec_status = tk.Label(btn_row2, text="", bg=BG_CARD,
                                    fg=ACCENT2, font=FONT_SMALL)
        ...
        tk.Label(rf1, text="Source:", bg=BG_CARD, fg=TEXT_SEC,
                 font=FONT_SMALL, width=8).pack(side="left")
        ...
        styled_btn(rf1, "Browse", lambda: self._browse_src(),
                   BG_INPUT, 8, small=True).pack(side="left", padx=(6,0))

        rf2 = tk.Frame(rc, bg=BG_CARD)
        rf2.pack(fill="x", padx=12, pady=(0,4))
        tk.Label(rf2, text="Dest:", bg=BG_CARD, fg=TEXT_SEC,
                 font=FONT_SMALL, width=8).pack(side="left")
        ...
        styled_btn(rf2, "Browse", lambda: self._browse_dst(),
                   BG_INPUT, 8, small=True).pack(side="left", padx=(6,0))

        btn_row3 = tk.Frame(rc, bg=BG_CARD)
        btn_row3.pack(fill="x", padx=12, pady=(4,12))
        styled_btn(btn_row3, "📦  Relocate (3-pass wipe)", self._relocate,
                   PURPLE, 26).pack(side="left")
        self._rel_status = tk.Label(btn_row3, text="", bg=BG_CARD,
                                    fg=ACCENT2, font=FONT_SMALL)
        ...
        self._file_list = tk.Listbox(
            vc, bg=BG_INPUT, fg=TEXT_PRI, font=FONT_MONO_S,
        ...
        styled_btn(vc, "🔄  Refresh", self._refresh_files,
                   BG_INPUT, 12, small=True).pack(anchor="w", padx=12, pady=(0,10))
        self._refresh_files()

    def _browse_encrypt(self):
        p = filedialog.askopenfilename(title="Select file to encrypt")
        if p: self._enc_path.set(p)

    def _browse_decrypt(self):
        p = filedialog.askopenfilename(title="Select .vault file",
                                       filetypes=[("Vault blobs","*.vault"),("All","*.*")])
        if p: self._dec_path.set(p)

    def _browse_src(self):
        p = filedialog.askopenfilename(title="Source .vault file",
                                       filetypes=[("Vault blobs","*.vault")])
        if p: self._rel_src.set(p)

    def _browse_dst(self):
        p = filedialog.asksaveasfilename(title="Destination path",
                                         defaultextension=".vault")
        if p: self._rel_dst.set(p)
    ...
    def _encrypt(self):
        ...
        try:
            ...
            self._enc_status.config(
                text=f"✓ Saved: {os.path.basename(out_path)}", fg=ACCENT2)
            self._app.ledger.log("ENCRYPT_GUI", {
                "file": os.path.basename(path), "blob_id": blob.blob_id[:16]
            })
            self._refresh_files()
        except Exception as e:
    ...
    def _decrypt(self):
        ...
        try:
            ...
            out_path = path.replace(".vault", ".decrypted") if path.endswith(".vault") else path + ".decrypted"
            with open(out_path, "wb") as f:
                f.write(plain)
            self._dec_status.config(
                text=f"✓ Saved: {os.path.basename(out_path)}", fg=ACCENT2)
            self._app.ledger.log("DECRYPT_GUI", {
    ...
    def _relocate(self):
        ...
        if not src or not dst:
            ...
            return
        try:
            registry  = LocationRegistry(self._app.gateway.config.setdefault("location_registry", {}))
...
class AuditPanel(tk.Frame):
    ...
    def _build(self):
        tk.Label(self, text="Audit Log", bg=BG_PANEL,
                 fg=TEXT_PRI, font=FONT_TITLE).pack(anchor="w", padx=20, pady=(20,4))
        tk.Label(self, text="Immutable hash-chained Ed25519-signed ledger",
                 bg=BG_PANEL, fg=TEXT_SEC, font=FONT_BODY).pack(anchor="w", padx=20, pady=(0,12))

        # Action row
        ar = tk.Frame(self, bg=BG_PANEL)
        ar.pack(fill="x", padx=20, pady=(0,12))
        styled_btn(ar, "🔄  Refresh", self._refresh, ACCENT, 14, small=True).pack(side="left", padx=(0,8))
        styled_btn(ar, "✅  Verify Chain", self._verify, ACCENT2, 16, small=True).pack(side="left", padx=(0,8))
        styled_btn(ar, "💾  Export", self._export, BG_INPUT, 12, small=True).pack(side="left")
        self._chain_lbl = tk.Label(ar, text="", bg=BG_PANEL,
                                   fg=ACCENT2, font=("Segoe UI",9,"bold"))
        self._chain_lbl.pack(side="left", padx=12)

        # Log view
        self._text = scrolledtext.ScrolledText(
            self, bg=BG_CARD, fg=TEXT_PRI, font=FONT_MONO_S,
        ...
        try:
            ...
            for e in reversed(entries):
                ...
                self._text.insert("end",
                    f"  prev: {e.prev_hash[:24]}...   hash: {e.entry_hash[:24]}...\n", "dim")
    ...
    def _verify(self):
        ...
        self._chain_lbl.config(
            text=f"{'✅' if valid else '❌'}  {msg}",
            fg=ACCENT2 if valid else DANGER
        )

    def _export(self):
        path = filedialog.asksaveasfilename(
            defaultextension=".json", title="Export audit ledger"
        )
        if path:
...
class ThreatMonitorPanel(tk.Frame):
    ...
    def _build(self):
        tk.Label(self, text="Threat Monitor", bg=BG_PANEL,
                 fg=TEXT_PRI, font=FONT_TITLE).pack(anchor="w", padx=20, pady=(20,4))
        tk.Label(self, text="Real-time copy detection and security alerts",
                 bg=BG_PANEL, fg=TEXT_SEC, font=FONT_BODY).pack(anchor="w", padx=20, pady=(0,12))

        # Daemon status bar
        self._daemon_bar = tk.Frame(self, bg=BG_CARD, bd=0,
                                    highlightthickness=1, highlightbackground=BORDER)
        self._daemon_bar.pack(fill="x", padx=20, pady=(0,12))
        self._daemon_lbl = tk.Label(self._daemon_bar,
                                    text="⬤  Checking daemon...",
                                    bg=BG_CARD, fg=WARN, font=("Segoe UI",10,"bold"))
        ...
        styled_btn(btn_row, "▶  Start Daemon",
                   self._start_daemon, ACCENT2, 16, small=True).pack(side="left", padx=4)
        styled_btn(btn_row, "■  Stop Daemon",
                   self._stop_daemon, DANGER, 14, small=True).pack(side="left", padx=4)
        ...
        styled_btn(ar, "🔄  Refresh", self._refresh_alerts,
                   BG_INPUT, 12, small=True).pack(side="left")
        styled_btn(ar, "🗑  Clear Log", self._clear_alerts,
                   BG_INPUT, 12, small=True).pack(side="left", padx=8)
        self._alert_count_lbl = tk.Label(ar, text="", bg=BG_CARD,
                                          fg=DANGER, font=("Segoe UI",9,"bold"))
        self._alert_count_lbl.pack(side="left", padx=8)

        self._alert_text = scrolledtext.ScrolledText(
            c, bg=BG_INPUT, fg=TEXT_PRI, font=FONT_MONO_S,
            relief="flat", bd=0, height=20, state="disabled"
        )
    ...
    def _update_daemon_status(self):
        try:
            ...
            self._daemon_lbl.config(
                text="⬤  Daemon: Not available", fg=TEXT_DIM
            )
    ...
    def _refresh_alerts(self):
        ...
        try:
            ...
            self._alert_count_lbl.config(
                text=f"{len(alerts)} alert(s)" if alerts else "No alerts"
            )
            if not alerts:
                self._alert_text.insert("end", "  ✅  No security alerts. Vault is clean.\n", "info")
            else:
...
class VaultXApp(tk.Tk):
    ...
    def _build_sidebar(self):
        # Logo
        tk.Label(self._sidebar, text="VAULT-X", bg=BG_PANEL,
                 fg=ACCENT, font=("Segoe UI",16,"bold")).pack(pady=(20,2))
        tk.Label(self._sidebar, text="v2.0.0", bg=BG_PANEL,
                 fg=TEXT_DIM, font=FONT_SMALL).pack(pady=(0,20))
        ...
        for key, label in nav_items:
            b = tk.Button(
                self._sidebar, text=label, bg=BG_PANEL,
                fg=TEXT_SEC, font=("Segoe UI",11), relief="flat",
        ...
        styled_btn(self._sidebar, "🔒  Logout", self._logout,
                   DANGER, 16, small=True).pack(side="bottom", padx=16, pady=12, fill="x")

        # Device info
        dh = get_device_fingerprint()
        tk.Label(self._sidebar, text=f"Device: {dh[:12]}...",
                 bg=BG_PANEL, fg=TEXT_DIM, font=("Segoe UI",8)).pack(
            side="bottom", padx=16, pady=(0,4))
    ...
    def show_panel(self, name: str):
        ...
        for k, b in self._nav_btns.items():
            b.config(
                bg=BG_CARD if k == name else BG_PANEL,
                fg=TEXT_PRI if k == name else TEXT_SEC
            )
    ...
    def _logout(self):
        if messagebox.askyesno("Logout", "End vault session and logout?"):
            ...
            if self.ledger:
                self.ledger.log("SESSION_CLOSED_GUI", {
